...
/datum/antagonist/bloodsucker/proc/break_masquerade(mob/admin)
	if(broke_masquerade)
		return
	owner.current.playsound_local(null, 'modular_zubbers/code/modules/antagonists/bloodsuckers/sounds/lunge_warn.ogg', 100, FALSE, pressure_affected = FALSE)
...
/datum/antagonist/bloodsucker/proc/return_vamp_examine(mob/living/viewer)
	...
	if(viewer.mind.has_antag_datum(/datum/antagonist/vassal) in vassals)
		...
		var/returnIcon = "[icon2html('modular_zubbers/code/modules/antagonists/bloodsuckers/icons/vampiric.dmi', world, "bloodsucker")]"
		returnString += "\n"
